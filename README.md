...
To get the polls associated with a specific chart, we use `pollster_chart_data()`. 
...
However, the `pollster_chart_data()` function correctly returns the poll data in a Pollster chart (see the [API docs](http://elections.huffingtonpost.com/pollster/api#chart-csv)).
